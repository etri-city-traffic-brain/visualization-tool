...
const error = {
  ...
  invalidate: Error('invalid time duration (check start and end time)'),
};
...
const makeMomentFactory = moment => dateTime =>
...
const isValid = (fromMoment, toMoment) =>
  new Promise((resolve, reject) => {
...
const getDates = (startMoment, endMoment, resultMoments = []) => {
  const nextMoment = moment(startMoment).add(1, 'day').hour(0).minute(0)
    .second(0);

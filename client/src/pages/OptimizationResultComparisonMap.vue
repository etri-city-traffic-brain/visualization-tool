    ...
    <div v-else>
        ...
        <div>
          <div class="flex items-center space-x-1">
            <select v-model="selectedEpoch" size="sm" class="text-black rounded py-1 px-2" style="height:30px">
              <option
                v-for="(reward, idx) of rewards.labels.filter((v,i)=> (i % simulation.configuration.modelSavePeriod) == 0)"
                :key="reward"
                :value="reward"
              >
            ...
            <b-btn @click.prevent="runTest" variant="secondary" size="sm" class="flex-none">
      ...
      <div class="text-white p-1 pt-2 flex justify-between border-b- border-gray-200">
        ...
        <div class="flex pr-2 space-x-2 text-sm items-center text-white">
          <!-- <div> {{  status }}</div> -->
          <!-- <div>{{ statusText }}</div> -->
          <button @click="showModal" class="bg-gray-400 px-1 text-black rounded hover:bg-blue-400">정보</button>
          <button @click="checkStatus" class="bg-gray-400 px-1 text-black rounded hover:bg-blue-400">상태 확인</button>
        </div>
      </div>
      ...
      <div class="relative min-w-max">
        ...
        <div class="grid grid-cols-4 gap-1 p-1">
          ...
          <div class="max-w-84 flex flex-column">
            ...
            <div class="m-1 space-y-1 flex-1 overflow-auto">
              <div class="p-1 pb-2 bg-gray-800 text-sm rounded h-0">
                <div class="text-sm bg-gray-800">
                  <div v-if="Object.keys(chart.travelTimePerJunction || {}).length > 0">
                    ...
                    >
                      ...
                      <div class="border-b text-right">{{ v[1].simulate.travel_time }}</div>
                      <div class="border-b text-right">{{ v[1].test.travel_time }}</div>
              ...
              <div>
                <div class="tracking-wide px-2 font-bold text-center text-white flex justify-between items-center">
                  ...
                  <div class="bg-gray-700 px-2">
                    <button @click="isShowAvgTravelChart = !isShowAvgTravelChart" class="text-sm">닫기</button>
                  </div>
                </div>
                  ...
                  <div class="grid grid-cols-4 gap-1">
                    ...
                    <div class="relative py-1">
                      ...
                      <div class="absolute top-2 left-2">
                        <div class="text-white text-sm">{{ selectedNode }}</div>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="absolute bottom-0 right-2 bg-gray-700 px-2" v-if="!isShowAvgTravelChart">
          <button @click="isShowAvgTravelChart = true" class="text-sm text-white">그래프</button>
        </div>

        <div v-if="showWaitingMsg" class="absolute top-24 w-full">
          <div class="flex justify-center">
            <div class="border rounded text-center bg-blue-300 p-5 text-xl font-bold flex items-center space-x-1 justify-center w-92">
        ...
        <div v-if="status==='error'" class="absolute top-24 w-full">
          <div class="flex justify-center">
            <div class="border rounded-xl text-center bg-red-400 p-5 text-xl font-bold w-92">
    ...
    >
      <div class="text-white text-sm p-2 min-w-max">
        <div class="border-blue-600 space-y-1">
          ...
          <div class="flex space-x-1">
            <div class="w-20 text-center bg-gray-500 px-1 rounded">보상함수</div>
            <div class="w-40">{{ config.rewardFunc }}</div>
            <div class="w-20 text-center bg-gray-500 px-1 rounded">통계주기</div>

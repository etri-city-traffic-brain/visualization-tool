<template>
  <div>
    <div class="uniq-top-left">
      <div
        class="bg-gray-700 py-2 text-white p-2 rounded-lg text-sm space-y-1 border w-80"
      >
        <div>
          <span
            class="w-14 inline-block text-center bg-gray-500 text-white rounded text-sm px-1 font-bold"
            >ID</span
          >
          {{ simulation.id }}
        </div>
        ...
        <div>
          ...
          >
          <span class="text-xs"
            >{{ simulation.configuration.dockerImage }}
          </span>
        </div>
        ...
        <div>
          ...
          >
          <span
            class="bg-gray-700 rounded-full text-center uppercase text-white font-bold space-x-1"
          >
            <span
              class="inline-block bg-green-500 px-1 animate-pulse rounded"
              v-if="
            ...
            <button
              class="bg-gray-600 text-white px-2 rounded text-sm hover:bg-blue-600"
              @click="getReward"
            >
      ...
      <div
        class="mt-2 p-2 bg-gray-700 font-bold text-white text-sm opacity-90 rounded-lg w-80"
      >
        <!-- 최적화 진행률 -->
        <div
          class="mb-1 uppercase inline-block text-center text-white rounded text-sm px-1 font-bold"
        >
      ...
      <div class="bg-gray-600 opacity-90 rounded-lg mt-2 p-2 w-80">
        <div>
          ...
          <button
            class="font-bold bg-yellow-300 px-2 rounded text-sm hover:bg-yellow-600 hover:text-white py-1"
            @click="stop"
            size="sm"
          >
    ...
    <!-- BOTTOM LEFT -->
    <!-- ----------- -->
    <div class="uniq-bottom-left"></div>
    ...
    <div class="reward-charts flex flex-col flex-wrap max-h-max rounded-lg">
      ...
      <div v-for="(chart, idx) of rewardCharts" :key="idx" class="p-1">
        <div
          class="w-80 text-center text-xs text-white px-2 pt-2 py-1 bg-gray-600 rounded-t-lg tracking-wide"
        >
          {{ chart.label }}
        </div>

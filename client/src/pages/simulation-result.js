...
export default {
  ...
  async mounted () {
    ...
    this.speedsPerStep.labels = new Array(this.speedsPerStep.labels.length)
      .fill(0)
      .map((v, i) => i)
    ...
    this.histogramDataStep = await statisticsService.getHistogramChart(
      simulationId,
      0
    )
    this.ready = true
  ...
  methods: {
    ...
    async loadSimulationInfo (simulationId) {
      const { simulation, slideMax } =
        await simulationService.getSimulationInfo(simulationId)
